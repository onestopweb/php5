<?php
	...
	//图片存放存处
	define('PRO_PATH',ROOT_PATH.'/product/');
	define('PRO_PATH_20',ROOT_PATH.'/product/20/');
	define('PRO_PATH_100',ROOT_PATH.'/product/100/');
	//上传
	function _uploads($_FILES){
		...
		if(is_uploaded_file($_FILES['userfile']['tmp_name'])){
			if(!move_uploaded_file($_FILES['userfile']['tmp_name'],PRO_PATH.$_img)){
				echo "<script>alert('上传失败');history.back();</script>";
				exit;
			}
		}
		//创建缩略图
		_create_thumbnail(PRO_PATH.$_img,20,20,PRO_PATH_20.$_img);
		_create_thumbnail(PRO_PATH.$_img,100,100,PRO_PATH_100.$_img);
		return $_img;
	}
	//创建压缩图
