<?php 
	...
	if($_GET['del_id']!=null){
		$_del_id = $_GET['del_id'];
		$_sql = "DELETE FROM o_liber WHERE o_valid = '0' AND o_id = $_del_id  LIMIT 1";
		_query($_sql);
	}
	if($_GET['rec_id']!=null){
		$_rec_id = $_GET['rec_id'];
		$_sql = "UPDATE o_liber SET o_valid = '1' WHERE o_id = $_rec_id LIMIT 1";
		_query($_sql);
	}
	if($_GET['del']=='all'){
		$_sql = "DELETE FROM o_liber WHERE o_valid = '0'";
		_query($_sql);
	}
	if($_GET['rec']=='all'){
		$_sql = "UPDATE o_liber SET o_valid = '1'";
		_query($_sql);
	}
?>
...
<tr class="search">
	<td colspan="9">
        <form action="valid.liber.php" method="get">
...
<?php 
	...
	$i++;
?>
	<tr class="item<?php if($i%2==0){ echo ' color_bg';}?>"><td><a href="../info.liber.php?id=<?php echo $row[o_id];?>" target="_blank"><?php echo $row[o_id];?></a></td><td><img src="../product/20/<?php echo $row[o_img];?>"></img></td><td style="width: 280px;"><a title="<?php echo $row[o_title] ?>"><?php echo mb_substr($row[o_title],0,18,'utf-8') ?></a></td><td><?php echo $row[o_good]?></td><td>
                <?php 
                	if ($row[o_valid]==1){
                		echo '<i>有效</i>';
                	}else{
