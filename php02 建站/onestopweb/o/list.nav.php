<?php 
	...
	if($_GET['oid']!=null){
		$_oid = $_GET['oid'];
		$_sql = "DELETE FROM o_nav WHERE o_id = $_oid LIMIT 1";
		_query($_sql);
	}
?>
...
<tr class="search">
	<td colspan="10">
        <form action="list.nav.php" method="get">
...
<?php 
	...
	$i++;
?>
	<tr class="item<?php if($i%2==0){ echo ' color_bg';}?>"><td><a href="../info.nav.php?id=<?php echo $row[o_id];?>" target="_blank"><?php echo $row[o_id];?></a></td><td><img src="../product/20/<?php echo $row[o_img];?>"></img></td><td style="width: 280px;"><a title="<?php echo $row[o_title] ?>"><?php echo mb_substr($row[o_title],0,18,'utf-8') ?></a></td><td><?php echo $row[o_see]?></td><td><?php echo $row[o_good]?></td><td>
				<?php 
                	if ($row[o_top]==1){
                		echo '<i>经典</i>';
                	}else{
                ...
                ?>
                </td><td><?php echo $row[o_user_id]?></td><td><?php echo $_type_arr[$row[o_type_id]-1]; ?></td><td><a href="?search=<?php echo $_search;?>&page=<?php echo $_page;?>&oid=<?php echo $row[o_id];?>" onclick="javascript:return confirm('您确定要删除该条数据吗？')"  title="删除该条数据">删除</a>&nbsp;&nbsp;<a href="update.nav.php?id=<?php echo $row[o_id];?>" title="修改或更新">编辑</a></td></tr>
<?php
	}
?>
<tr class="page">
    <td colspan="10">
        <span class="f_l"><a href="add.nav.php">发布新贴</a></span>
<?php
$_con = '';
$_sql_count = "SELECT count( o_id ) FROM o_nav "; 
if($_search !=null){
