<?php 
	...
	if($_GET['del_id']!=null){
		$_del_id = $_GET['del_id'];
		$_sql = "DELETE FROM o_nav_link WHERE o_valid = '0' AND o_id = $_del_id LIMIT 1";
		_query($_sql);
	}
	if($_GET['rec_id']!=null){
		$_rec_id = $_GET['rec_id'];
		$_sql = "UPDATE o_nav_link SET o_valid = '1' WHERE o_id = $_rec_id LIMIT 1";
		_query($_sql);
	}
	if($_GET['del']=='all'){
		$_sql = "DELETE FROM o_nav_link WHERE o_valid = '0'";
		_query($_sql);
	}
	if($_GET['rec']=='all'){
		$_sql = "UPDATE o_nav_link SET o_valid = '1'";
		_query($_sql);
	}
?>
...
<tr class="search">
	<td colspan="9">
        <form action="link.nav.php" method="get">
...
<?php 
	$_sql_list = "SELECT * FROM o_nav_link ";
	if($_search !=null){
	...
	$i++;
?>
	<tr class="item<?php if($i%2==0){ echo ' color_bg';}?>"><td><a href="../info.nav.php?id=<?php echo $row[o_main_id];?>" title="<?php echo $row[o_title] ?>" target="_blank"><?php echo $row[o_main_id];?></a></td><td style="width: 440px;"><a href="<?php echo $row[o_link] ?>" title="<?php echo $row[o_link] ?>" target="_blank"><?php echo mb_substr($row[o_link],0,50,'utf-8') ?></a></td><td><?php echo $row[o_good]?></td><td>
                <?php 
                	if ($row[o_valid]==1){
                		echo '<i>有效</i>';
                	}else{
