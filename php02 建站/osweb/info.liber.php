<?php 
	...
	if($_id == null || !(is_numeric($_id))){
		_close();
		echo "<script type='text/javascript'>alert('无效访问');location.href='search.liber.php';</script>";
		exit();
	}
	$_sql = " SELECT * FROM o_liber WHERE o_id = $_id LIMIT 1";
	$_info = _fetch_array($_sql);
	if(!$_info){
		$_sql = " SELECT * FROM o_liber WHERE o_id = 1 LIMIT 1";
		$_info = _fetch_array($_sql);
	}
	if ($_GET['links'] == 'onestopweb') {
		...
		$_sql = "INSERT INTO o_liber_link ( o_link , o_time ,o_good ,o_valid ,o_main_id ,o_user_id)
				VALUES ( '$_link', NOW(), '1', '1', '$_id', '$_user_id');";
		if(_query($_sql)){
			_close();
			echo "<script type='text/javascript'>alert('提交链接成功');location.href='info.liber.php?id=$_id';</script>";
			exit();
		}
	}
	if($_GET['cons'] == 'onestopweb'){
		...
		$_sql = "INSERT INTO o_liber_con (o_con ,o_time ,o_good ,o_valid ,o_main_id,o_user_id)
				VALUES ('$_con', NOW(), '1', '1', '$_id', '$_user_id')";
		if(_query($_sql)){
			_close();
			echo "<script type='text/javascript'>alert('提交内容成功');location.href='info.liber.php?id=$_id';</script>";
			exit();
		}
	}
	//统计点击次数
	$_see_number = $_info['o_see']+1;
	$_sql_see = "UPDATE o_liber SET o_see = '$_see_number' WHERE o_id = $_id LIMIT 1";
	_query($_sql_see);
?>
    ...
    <div class="search c">
    	...
    	<div class="search_select f_l">
            <select name="option">
                ...
                <option value="4" selected="selected">书籍文档</option>
...
<div class="nav">
	<p>
        ...
        <a href="search.liber.php" class="navNow f_l">书籍文档</a>
...
<div class="result">
    ...
    <p class="f_l">当前位置：<a href="index.php">首页</a> &gt; <a href="search.liber.php">书籍文档</a> &gt; <a href="search.liber.php?type=<?php echo $_info[o_type_id]; ?>"><?php echo $_type_arr[$_info[o_type_id]-1]; ?></a>
...
<div class="cont">
	...
	<div class="show">
    	<h3>
            ...
            <i class="f_r" onclick="javascript:good(<?php echo $_info[o_id]?>,<?php echo $_info[o_good]?>,4);" id="good"><?php echo $_info['o_good'] ?>个赞</i>
            <span><?php echo mb_substr($_info[o_title],0,30,'utf-8')?></span>
            <?php 
                if ($_info[o_valid]==1){
            ?>
            	<a class="valid" onclick="javascript:valid(<?php echo $_info[o_id]?>,4);" id="valid">有效</a>
            <?php
                }else{
...
<?php
	$_sql_img = "SELECT * FROM o_liber_img WHERE o_main_id =$_id LIMIT 5";
        ...
        <div class="show_link">
        	<table border="0" cellpadding="0" cellspacing="0">
            	<tr>
                    ...
                    <td><em onclick="javascript:good2(<?php echo $_info[o_id]?>,<?php echo $_info[o_good]?>,4);" id="good2"><?php echo $_info['o_good'] ?>个赞</em></td>
                    <td>链接错误请点击 -&gt;
	                <?php 
		                if ($_info[o_valid]==1){
		            ?>
		            	<a class="valid" onclick="javascript:valid2(<?php echo $_info[o_id]?>,4);" id="valid2">有效</a>
		            <?php
		                }else{
		                	echo '<a class="lose">处理中</a>';
		                }
...
<?php
	$_sql_link = " SELECT * FROM o_liber_link WHERE o_main_id =$_id LIMIT 0 , 30 ";
...
?>
               <tr>
                    ...
                    <td><em onclick="javascript:good_link(<?php echo $row[o_id]?>,<?php echo $row[o_good]?>,4);" id="good_link<?php echo $row[o_id] ?>"><?php echo $row[o_good] ?>个赞</em></td>
                    <td>链接错误请点击 -&gt;
	                <?php 
		                if ($row[o_valid]==1){
		            ?>
		            	<a class="valid" onclick="javascript:valid_link(<?php echo $row[o_id]?>,4);" id="valid_link<?php echo $row[o_id] ?>">有效</a>
		            <?php
		                }else{
		                	echo '<a class="lose">处理中</a>';
		                }
...
<?php }?>
        	...
        	<form method="post" name="linkForm" action="info.liber.php?id=<?php echo $_id?>&links=onestopweb">
...
<?php
	$_sql_con_count = "SELECT count(o_id) FROM o_liber_con WHERE o_main_id=$_id";
...
<?php
	$_sql_con = "SELECT * FROM o_liber_con WHERE o_main_id=$_id ";
	$_sort = $_GET['sort'];
...
?>
        <dl>
            ...
            <dd class="line03">发布时间：<?php echo $row[o_time]?> &nbsp;&nbsp;&nbsp;&nbsp;<em  onclick="javascript:good_cont(<?php echo $row[o_id]?>,<?php echo $row[o_good]?>,4);" id="good_cont<?php echo $row[o_id] ?>"><?php echo $row[o_good]?>个赞</em>&nbsp;&nbsp;&nbsp;&nbsp; 内容不好请点击 -&gt;
				 <?php 
		       		if ($row[o_valid]==1){
		       	?>
		        <a class="valid" onclick="javascript:valid_cont(<?php echo $row[o_id]?>,4);" id="valid_cont<?php echo $row[o_id] ?>">有效</a>
		        <?php
		        	}else{
		            	echo '<a class="lose">处理中</a>';
		            }
...
?>
        <form method="post" action="info.liber.php?id=<?php echo $_id?>&cons=onestopweb">
        <div class="show_say">
        	<?php
        		if(isset($_COOKIE['name'])){
