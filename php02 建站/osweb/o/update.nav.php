<?php 
	...
	if($_GET['id']!=null){
		...
		if ($_GET['action'] == 'onestopweb') {
			...
			_close();
			echo "<script type='text/javascript'>alert('修改成功');location.href='update.nav.php?id=$_id';</script>";
			exit();
		}
		if ($_GET['action'] == 'uploads') {
			...
			if($_FILES[userfile][name]){
				$_img = _uploads($_FILES);
				$_sql = "INSERT INTO o_nav_img (o_img ,o_time ,o_main_id ,o_user_id)
				VALUES ('$_img', NOW(), '$_id', '$_name')";
				_query($_sql);
				_close();
				echo "<script type='text/javascript'>location.href='update.nav.php?id=$_id';</script>";
				exit();
			}
		}
		if($_GET['del']!=null){
			$_del = $_GET['del'];
			$_sql = "DELETE FROM o_nav_img WHERE o_id = $_del LIMIT 1";
			_query($_sql);
		}
	}else{
		_close();
...
<div class="update">
<form action="update.nav.php?id=<?php echo $_id?>&action=onestopweb" method="post" enctype="multipart/form-data">
...
<div class="img">
<form action="update.nav.php?id=<?php echo $_id?>&action=uploads" method="post" enctype="multipart/form-data">
<table cellpadding="0" cellspacing="0">
	...
	<tr>
        ...
        <td>
        	<?php
				$_sql_img = "SELECT * FROM o_nav_img WHERE o_main_id = $_id";
