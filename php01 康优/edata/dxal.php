	...
	<div class="list">
		<ul>
			<?php
				...
				$_sql_list = "SELECT * FROM e_dxal order by e_time DESC LIMIT $_page_begin , $_page_size";
				$_result = _query($_sql_list);
				while (!!$row = mysql_fetch_array($_result)){
					echo '<li><a href="dxal.cont.php?id='.$row[e_id].'">'.$row[e_title].'</a></li>';
				}
			?>
		</ul>
	</div>
