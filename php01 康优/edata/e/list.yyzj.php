<?php 
	...
	if($_GET['eid']!=null){
		$_eid = $_GET['eid'];
		$_sql = "DELETE FROM e_yyzj WHERE e_id = $_eid LIMIT 1";
		_query($_sql);
	}
?>
...
<tr class="search">
	<td colspan="7">
        <form action="list.yyzj.php" method="get">
...
<tr class="head">
	<td>编号</td><td>图片</td><td>标题</td><td>阅读量</td><td>发布人</td><td>发布时间</td><td>操作</td></tr>
<?php 
	...
	$i++;
?>
	<tr class="item<?php if($i%2==0){ echo ' color_bg';}?>"><td><a href="../yyzj.cont.php?id=<?php echo $row[e_id];?>" target="_blank"><?php echo $row[e_id];?></a></td><td><img src="../product/20/<?php echo $row[e_img];?>"></img></td><td style="width: 360px;"><a title="<?php echo $row[e_title] ?>" href="../yyzj.cont.php?id=<?php echo $row[e_id];?>" target="_blank"><?php echo mb_substr($row[e_title],0,28,'utf-8') ?></a></td>
	<td><?php echo $row[e_see]?></td><td><?php echo $row[e_name]?></td><td><?php echo substr($row[e_time],0,10) ?></td><td><a href="?search=<?php echo $_search;?>&page=<?php echo $_page;?>&eid=<?php echo $row[e_id];?>" onclick="javascript:return confirm('您确定要删除该条数据吗？')"  title="删除该条数据">删除</a>&nbsp;&nbsp;<a href="update.yyzj.php?id=<?php echo $row[e_id];?>" title="修改或更新">编辑</a></td></tr>
<?php
	}
?>
	<tr class="page">
    	<td colspan="7">
		<span class="f_l"><a href="add.yyzj.php">增加信息</a></span>
<?php
$_con = '';
