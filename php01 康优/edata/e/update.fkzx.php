<?php
	...
	if($_GET['id']!=null){
		...
		if ($_GET['action'] == 'edata') {
			...
			_query($_sql_update);
			_close();
			echo "<script type='text/javascript'>location.href='list.fkzx.php';</script>";
			exit();
		}
	}else{
		_close();
...
<div class="update">
<form action="update.fkzx.php?id=<?php echo $_id?>&action=edata" method="post">
<table cellpadding="0" cellspacing="0">
    ...
    <tr><td class="h38 b">选择分类：</td>
    	<td>
    		<span>
    			<?php
			    	for($i=0;$i<count($_type_arr);$i++){
						if(($i+1)==$_result[e_type]){
