<?php 
	...
	if($_GET['eid']!=null){
		$_eid = $_GET['eid'];
		$_sql = "DELETE FROM e_tsjs WHERE e_id = $_eid LIMIT 1";
		_query($_sql);
	}
?>
...
<tr class="search">
	<td colspan="6">
        <form action="list.tsjs.php" method="get">
...
<?php 
	...
	$i++;
?>
	<tr class="item<?php if($i%2==0){ echo ' color_bg';}?>"><td><a href="../tsjs.cont.php?id=<?php echo $row[e_id];?>" target="_blank"><?php echo $row[e_id];?></a></td><td style="width: 360px;"><a title="<?php echo $row[e_title] ?>" href="../tsjs.cont.php?id=<?php echo $row[e_id];?>" target="_blank"><?php echo mb_substr($row[e_title],0,28,'utf-8') ?></a></td>
	<td><?php echo $row[e_see]?></td><td><?php echo $row[e_name]?></td><td><?php echo substr($row[e_time],0,10) ?></td><td><a href="?search=<?php echo $_search;?>&page=<?php echo $_page;?>&eid=<?php echo $row[e_id];?>" onclick="javascript:return confirm('您确定要删除该条数据吗？')"  title="删除该条数据">删除</a>&nbsp;&nbsp;<a href="update.tsjs.php?id=<?php echo $row[e_id];?>" title="修改或更新">编辑</a></td></tr>
<?php
	}
?>
	<tr class="page">
    	<td colspan="6">
		<span class="f_l"><a href="add.tsjs.php">增加信息</a></span>
<?php
$_con = '';
$_sql_count = "SELECT count( e_id ) FROM e_tsjs "; 
if($_search !=null){
