<?php 
	...
	if($_id == null || !(is_numeric($_id))){
		_close();
		echo "<script type='text/javascript'>alert('无效访问');location.href='tsjs.php';</script>";
		exit();
	}
	$_sql = "SELECT * FROM e_tsjs WHERE e_id = $_id LIMIT 1";
	$_info = _fetch_array($_sql);
	if(!$_info){
		$_sql = "SELECT * FROM e_tsjs WHERE e_id = 1 LIMIT 1";
		$_info = _fetch_array($_sql);
	}
	$_see_number = $_info['e_see']+1;
	$_sql_see = "UPDATE e_tsjs SET e_see = '$_see_number' WHERE e_id = $_id LIMIT 1";
	_query($_sql_see);
?>
<!doctype html>
<html lang="en">
...
?>
	<div class="index">
		<div class="head"><a href="tsjs.php">返回特色技术</a></div>
	</div>
	<h1 id="title">
	...
	<div class="list">
		<ul>
			<?php
			$_sql_list = "SELECT * FROM e_tsjs ORDER BY e_see DESC LIMIT 5";
